...
- **Halves and neutral script.** Shotgun rate over a whole game is partly a
  ...
  The table is sorted by `neutral_rate`; read `h1_rate` next to it as the
  larger-sample check, and `h2_minus_h1` as what the game did to the plan.
  Six teams had fewer than 20 neutral snaps in week 1 - CLE 8, BAL 10, MIA and
  IND 11, TB and JAX 17 - so their neutral rate turns on two or three calls.
